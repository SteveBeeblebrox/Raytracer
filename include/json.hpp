...
namespace json {
	...
	/// Shortcut for accessing values in a parsed JSON via lookup path, throws on error
	template<typename T> inline T get(const std::any& root, const std::string& path) {
		if(path.length()) {
			if(path.at(0) == '[') {
				if(const std::string::size_type pos = path.find(']'); pos != std::string::npos) {
					if(root.type() == typeid(std::vector<std::any>)) {
						return get<T>(std::any_cast<std::vector<std::any>>(root).at(std::stoi(path.substr(1, pos - 1))), path.substr(pos + 1));
					} else {
						throw std::runtime_error("Value is not a list");
					}
				} else {
					throw std::runtime_error("Unexpected end of string, unclosed '['");
				}
			} else if(path.at(0) == '.') {
				return get<T>(root, path.substr(1));
			} else {
				const std::string::size_type pos = std::min(path.find('.'), path.find('['));
				if(root.type() == typeid(std::map<std::string, std::any>)) {
					return get<T>(std::any_cast<std::map<std::string, std::any>>(root).at(pos == std::string::npos ? path : path.substr(0, pos)), pos == std::string::npos ? "" : path.substr(pos));
				} else {
					throw std::runtime_error("Value is not an object");
				}
			}
		} else {
			return std::any_cast<T>(root);
		}
	}

	/// Shortcut for accessing values in a parsed JSON via lookup path, returns fallback on error
	template<typename T> inline T get(const std::any& root, const std::string& path, T&& fallback) noexcept {
		try {
